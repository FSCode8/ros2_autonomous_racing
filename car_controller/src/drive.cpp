...
public:
  ...
  {
    // Parameter für den Controller-Typ (bicycle oder ackermann)
    ...
    // Publisher-Topic basierend auf controller_type bestimmen
    std::string topic;
    if (controller_type == "bicycle") {
      ...
      RCLCPP_WARN(this->get_logger(), 
                 "Unbekannter controller_type '%s', verwende 'bicycle' als Fallback", 
    ...
    RCLCPP_INFO(this->get_logger(), "Drive Node gestartet. Lauscht auf 'platzhalter/reference' und publiziert auf '%s'.", 
                topic.c_str());
